...
var (
	...
	sensitiveWords = []string{
		"password", "passwd", "psw", "pass",
		"api[_-]?key",
		"token", "secret", "ssn", "credit", "card", "cardnumber",
		"private[_-]?key",
	}
	sensitiveRe = regexp.MustCompile(`(?i)(?:^|\s|[:=_-])(?:` + strings.Join(sensitiveWords, "|") + `)(?:\s*[:=]|\s+as\s+|\s+is\s+)`)

	possibleLoggers = []string{"log/slog", "go.uber.org/zap"}
	allowedFuncs    = []string{"Info", "Error", "Warn", "Debug"}
)
...
func extractStringLiterals(expr ast.Expr) []StringLit {
	var result []StringLit

	switch v := expr.(type) {
	case *ast.BasicLit:
		if v.Kind == token.STRING {
			result = append(result, StringLit{
				Value: unquoteString(v.Value),
				Pos:   v.Pos(),
			})
		}
	...
	case *ast.BinaryExpr:
		if v.Op == token.ADD {
			result = append(result, extractStringLiterals(v.X)...)
			result = append(result, extractStringLiterals(v.Y)...)
		}
	}
...
func unquoteString(str string) string {
	unquotedSrt, err := strconv.Unquote(str)
	if err != nil {
		return str
	}
...
func isLogFunctionCall(pass *analysis.Pass, call *ast.CallExpr) bool {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return false
	}

	funcObj := pass.TypesInfo.ObjectOf(sel.Sel)
	if funcObj == nil || funcObj.Pkg() == nil {
		return false
	}

	return slices.Contains(possibleLoggers, funcObj.Pkg().Path()) && slices.Contains(allowedFuncs, funcObj.Name())
}

func checkRules(pass *analysis.Pass, pos token.Pos, msg string) {
	if msg == "" {
		return
	}

	if ok, issue := checkFirstLetter(msg); ok {
		pass.Reportf(pos, "%q: %s", msg, issue)
		return
	}

	if ok, issue := checkLatinOnly(msg); ok {
		pass.Reportf(pos, "%q: %s", msg, issue)
		return
	}
	...
	if ok, issue := checkSpecialChars(lowerMsg); ok {
		pass.Reportf(pos, "%q: %s", msg, issue)
		return
	}

	if ok, issue := checkEmoji(lowerMsg); ok {
		pass.Reportf(pos, "%q: %s", msg, issue)
		return
	}

	if ok, issue := checkSensitiveWords(lowerMsg); ok {
		pass.Reportf(pos, "%q: %s", msg, issue)
		return
	}
}

func checkFirstLetter(msg string) (bool, string) {
	if unicode.IsUpper([]rune(msg)[0]) {
		return true, "log should start with a lowercase letter"
	}
	return false, ""
}

func checkLatinOnly(msg string) (bool, string) {
	for _, letter := range msg {
		if unicode.IsLetter(letter) && !unicode.In(letter, unicode.Latin) {
			return true, "log should contain only English letters"
		}
	}
	return false, ""
}

func checkSpecialChars(msg string) (bool, string) {
	if specialCharsRe.MatchString(msg) {
		return true, "log contains disallowed special characters"
	}
	return false, ""
}

func checkEmoji(msg string) (bool, string) {
	if emojiRe.MatchString(msg) {
		return true, "log should not contain emoji"
	}
	return false, ""
}

func checkSensitiveWords(msg string) (bool, string) {
	if sensitiveRe.MatchString(msg) {
		return true, "log should not contain sensitive data"
	}
	return false, ""
}
